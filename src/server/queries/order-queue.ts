...
export async function getOrderQueue(
  ...
  const { data, error } = await supabase
    .from("orders")
    .select(
      "id, customer_name, customer_phone, room_number, delivery_method, status, total, notes, created_at, order_items(product_name_snapshot, quantity), payments(id, status, claimed_amount, transaction_id)",
    )
